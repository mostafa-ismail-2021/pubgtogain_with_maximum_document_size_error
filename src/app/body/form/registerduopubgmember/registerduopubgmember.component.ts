...
export class RegisterduopubgmemberComponent implements OnInit ,OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.getIp = this.duoBodyService.getIpAddress().subscribe((ipData:{ip})=>{
      ...
      this.checkVideoIdAndRegisterType = this.duoBodyService.getMemberVideoIdAndRegisterType().subscribe((data:{register:boolean , videoId:string , videoType:string , adAppearanceCountry:string}) =>{
        let _this = this;
        setTimeout(function () {
  ...
  {//console.log(form);form.form.controls.Email.status = "INVALID";form.form.status = "INVALID"
    if(this.click <= 6){
      ...
      this.getEmail = this.duoBodyService.getMemberEmail(
        //form.value.userName,
    ...
    if(userName.valid){
      if(this.oldUserNameValue != filteUserName)
      {
        ...
        this.timeOutUserfullName = setTimeout(function () {
          ...
          $this.checkUserfullName =   $this.duoBodyService.checkMemberUserName($this.oldUserNameValue).subscribe((userNameType)=>{
    ...
    if(gmailAccount.valid){
      if(this.oldGmailAccountValue != filteGmailAccont)
      {
        ...
        this.timeOutGmailAccount = setTimeout(function () {
          ...
          $this.checkGmailAccount =  $this.duoBodyService.checkMemberGmailAccount($this.oldGmailAccountValue).subscribe((gmailAccountType)=>{
